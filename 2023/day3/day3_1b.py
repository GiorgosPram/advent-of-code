...
def sum_adjacent_numbers(row, col, grid):
    ...
    for dr, dc in directions:
        ...
        if is_valid(new_row, new_col, grid):
            if grid[new_row][new_col].isdigit():
                total += int(grid[new_row][new_col])

    return total

def sum_part_numbers(schematic):
    lines = schematic.split('\n')
    grid = [list(line) for line in lines if line.strip()]  # Convert the schematic into a grid

    total_sum = 0
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            if grid[i][j].isdigit():
                total_sum += sum_adjacent_numbers(i, j, grid)
...
result = sum_part_numbers(example_schematic)
